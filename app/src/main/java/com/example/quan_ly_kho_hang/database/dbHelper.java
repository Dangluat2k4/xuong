...
public class dbHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE Bill(id integer PRIMARY KEY AUTOINCREMENT,productID integer REFERENCES Product(id),name text,quantity text REFERENCES Product(quantity),createByUser text REFERENCES Product(userID) ,createdDate text,note text)");//, foreign key (productID) references Product(id)
        db.execSQL("CREATE TABLE BillDetail(id integer PRIMARY KEY AUTOINCREMENT,billID integer REFERENCES Bill(id),quantity text REFERENCES Bill(quantity),createdDate text REFERENCES Bill(createdDate),price text REFERENCES Product(price))");
        ...
        db.execSQL("insert into Bill values (1,'1','Hóa đơn A', '10', 'admin', '2023-10-18 11:15:00', 'Hóa đơn trả hàng'),(2,'2', 'Hóa đơn B', '15', 'thukho', '2023-10-18 11:30:00', 'Bán hàng cho khách hàng')");
